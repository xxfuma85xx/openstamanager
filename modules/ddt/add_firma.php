...
if (get('anteprima') !== null) {
    // Lettura dati ddt
    $query = 'SELECT numero_esterno FROM dt_ddt WHERE id='.prepare($id_record);
    $rs = $dbo->fetchArray($query);

    if (empty($rs)) {
        echo tr('DDT inesistente!');
        exit();
    }

    // Gestione della stampa
    $filename_ = sanitizeFilename($rs[0]['numero_esterno'].'.pdf');
    $filename = $docroot.'/files/ddt/'.$filename_;
    $id_print = setting('Stampa per anteprima e firma DDT');
